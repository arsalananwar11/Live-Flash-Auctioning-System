...
broadcast_gateway = boto3.client('apigatewaymanagementapi', endpoint_url=os.getenv('BROADCAST_ENDPOINT'))
...
def broadcast_leaderboard(auction_id, connection_id):
    ...
    try:
        ...
        try:
            logger.info("Sending leaderboard update to connection_id: %s", connection_id)
...
def fetch_leaderboard(auction_id):
    ...
    try:
        ...
        logger.info("Fetched leaderboard for auction_id %s: %s", auction_id, leaderboard_sorted)
        return leaderboard_sorted
    except ClientError as e:
        logger.error("Failed to fetch leaderboard from table: %s", e.response['Error']['Message'], exc_info=True)
        return []
